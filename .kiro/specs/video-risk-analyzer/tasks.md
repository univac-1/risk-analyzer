...
- [x] 5. Gemini統合動画解析機能
- [x] 5.1 Geminiネイティブ動画入力による統合解析の実装
  - Gemini API（Vertex AI）のネイティブ動画入力機能で動画全体を一括解析
  - 動画内に表示されるテキストを抽出し、タイムコードを付与
  - Geminiから提供されるテキストの信頼度を考慮し、閾値以下の低信頼度テキストをリスク評価から除外
  - 抽出テキストの日本語比率を判定し、一定以下の比率のテキストをリスク評価から除外
  - テキストなし検出時のグレースフルハンドリング（テキストなしとして記録し、映像解析を継続）
  - 映像内の人物・物体・行動の検出とタイムコード付与
  - 表情・ジェスチャー・服装などの視覚的特徴の記録（Geminiの出力に応じて調整）
  - シーン状況（場所・雰囲気・コンテキスト）の分類（Geminiの出力に応じて調整）
...
- [x] 6.2 4観点リスク検出ロジックの実装
  - 攻撃性の検出（匿名性・拡散性・感情的反応・集団心理・個人攻撃）
  - 差別性の検出（人種・性別・性的指向への偏見）
  - 誤解を招く表現の検出（断定・曖昧・誇張・ステレオタイプ）
  - 迷惑行為・不衛生行為の検出（店舗・施設・公共の場での不適切な行為、器物損壊、食品・商品への汚損）
  - _Requirements: 5.5, 5.6, 5.7, 5.8_

- [x] 6.3 リスクスコアリングと根拠生成
  - 各リスク箇所へのスコア（重要度）算出
  - リスク判断の具体的根拠を記述
  - タイムコードとソース（音声/映像/動画内テキスト）の紐付け
  - _Requirements: 5.3, 5.4_

- [x] 7. 解析オーケストレーション
- [x] 7.1 並列解析パイプラインの実装
  - AudioAnalyzer、VideoAnalyzerの2つの解析器を並列実行
  - 各解析の完了待機と結果集約
  - フェーズ別進捗更新（audio, gemini_video_ocr, risk）
...
- [x] 7.3 エラーハンドリングと部分失敗対応
  - 個別解析のエラー時に他解析を継続
  - 失敗時のリトライ処理
  - エラー詳細の記録とステータス更新
  - _Requirements: 2.3, 3.5, 7.4, 8.6_
...
- [x] 8.4 SSEによるリアルタイム進捗配信の実装
  - Server-Sent Events エンドポイント
  - 進捗更新をリアルタイムでクライアントに配信
  - _Requirements: 7.1_

- [x] 9. フロントエンドUI実装
- [x] 9.1 動画アップロードコンポーネントの実装
...
- [x] 9.2 (P) 進捗表示コンポーネントの実装
  - 全体進捗率の表示
  - 各解析フェーズ（音声・Gemini統合解析・リスク評価）の個別進捗
  - 推定残り時間の表示
  - エラー発生時の表示と再試行オプション
  - _Requirements: 7.1, 7.2, 7.3, 7.4_

- [x] 9.3 (P) 結果表示コンポーネントの実装
  - リスク箇所のタイムコード順一覧表示
...
- [x] 9.4 (P) 動画プレビュー機能の実装
  - リスク箇所クリック時に該当タイムコードから再生
  - 動画プレーヤーコントロール
  - _Requirements: 6.4_

- [x] 9.5 ジョブ一覧コンポーネントの実装
  - 過去の解析ジョブ一覧表示
...
- [x] 10.2 統合テストの実装
  ...
  - 外部API（Speech-to-Text, Gemini API）のモックテスト
